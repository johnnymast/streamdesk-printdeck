 ...
 *
 * @param {string} context The context this function is called in.
 * @param {string} url The url to open.
 */
const openUrl = (context, url) => {
  if (StreamDeck.socket) {
    let json = {
      'event': 'openUrl',
      'payload': {
 ...
 */
const setTitle = (context, title, state = null) => {
  if (StreamDeck.socket) {
    let json = {
      'event': 'setTitle',
      'context': context,
...
/**
 * Set an image for the button for a given state.
 *
 * @param {string} data Base64 encoded image data.
 * @param {number} state The state (0 for down 1 for up).
 */
const setImage = (context, data, state) => {
  if (StreamDeck.socket) {
    let json = {
      'event': 'setImage',
      'context': context,
